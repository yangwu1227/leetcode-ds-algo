...
def max_bugdet(
    charge_times: MutableSequence[int], running_costs: MutableSequence, budget: int
) -> int:
    """
    ...
    for right in range(n):
        ...
        print(
            f"Processing index charge_times[{right}] = {charge_times[right]} and running_costs[{right}] = {running_costs[right]}"
        )
        print("Deque is empty") if not monotonic_nonincreasing_deque else print(
            f"Deque is non-empty and (back of the deque charge time: {charge_times[monotonic_nonincreasing_deque[-1]]}) < (current charge time: {charge_times[right]})? {charge_times[monotonic_nonincreasing_deque[-1]] < charge_times[right]}"
        )
        while (
            monotonic_nonincreasing_deque
            and charge_times[monotonic_nonincreasing_deque[-1]] < charge_times[right]
        ):
            print(
                f"  Popping index {monotonic_nonincreasing_deque[-1]} since (back of the deque charge time: {charge_times[monotonic_nonincreasing_deque[-1]]}) < (current charge time: {charge_times[right]})"
            )
        ...
        k += 1
        print("Deque is empty") if not monotonic_nonincreasing_deque else print(
            f"Deque is non-empty and (total = {(charge_times[monotonic_nonincreasing_deque[0]] + k * sum_running_cost)}) >= (budget = {budget})"
        )
        while (
            monotonic_nonincreasing_deque
            ...
            print(
                f"  The front of the deque is no longer in the window, pop index {monotonic_nonincreasing_deque[0]}"
            ) if monotonic_nonincreasing_deque[0] == left else print(
                f"  The front of the deque index {monotonic_nonincreasing_deque[0]} is still in the window, no pop"
            )
            if monotonic_nonincreasing_deque[0] == left:
                monotonic_nonincreasing_deque.popleft()
            print(
                f"  Sum of running costs minus {running_costs[left]} = {sum_running_cost - running_costs[left]} | k = {k - 1} | left = {left + 1}"
            )
...
def main() -> int:
    ...
    for charge_times, running_costs, budget in data:
        ...
        print(
            f"Charge times = {charge_times} | Running costs = {running_costs} | budget = {budget} ---> Max machine(s) = {max_machines}\n"
        )
