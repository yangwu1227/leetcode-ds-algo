...
def num_valid_subarrays(nums: MutableSequence[int]) -> int:
    ...
    for i in range(n):
        ...
        print("Stack is empty") if not monotonic_increasing else print(
            f"Stack is non-empty and (top of the stack: {nums[monotonic_increasing[-1]]}) > (current element: {nums[i]})? {nums[monotonic_increasing[-1]] > nums[i]}"
        )
        while monotonic_increasing and nums[monotonic_increasing[-1]] > nums[i]:
            print(
                f"Popping index {monotonic_increasing[-1]} since (top of the stack: {nums[monotonic_increasing[-1]]}) > (current element: {nums[i]})"
            )
        ...
        print(
            f"Stack - Indices: {monotonic_increasing} - Elements: {[nums[i] for i in monotonic_increasing]}"
        )
