...
def remove_k_digits(num: str, k: int) -> str:
    ...
    for digit in num:
        ...
        while digit_stack and k > 0 and digit_stack[-1] > digit:
            print(
                f"  Popping digit = {digit_stack[-1]} from stack since (top of stack: {digit_stack[-1]}) > (current digit: {digit}) | Decrement k = {k} -> {k - 1}"
            )
            digit_stack.pop()
            k -= 1
    ...
    while k > 0:
        print(
            f"  Popping digit = {digit_stack[-1]} since (k = {k}) > 0 | Decrement k = {k} -> {k - 1}"
        )
        digit_stack.pop()
...
def main() -> int:
    ...
    for num, k in data:
        ...
        print(
            f"Given num = {num}, the smallest possible number after removing {k} digits is {output}\n"
        )
