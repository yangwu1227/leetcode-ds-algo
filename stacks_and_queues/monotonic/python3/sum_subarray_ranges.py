...
def sum_subarray_ranges(nums: MutableSequence[int]) -> int:
    ...
    print("For each nums[i], compute its minimum span and subtract it from the output")
    for right_span_index in range(n + 1):
        print(
            f"  Processing index {right_span_index}, which is {nums[right_span_index] if right_span_index < n else 'end of array'}"
        )
        print(
        ...
        while monotonic_increasing and (
            ...
            ) if monotonic_increasing else print(
                f"    Stack is empty so span of {nums[top_stack_i]} starts at -1"
            )
            print(
                f"    output -= {nums[top_stack_i]} * ({top_stack_i} - {left_span_index}) * ({right_span_index} - {top_stack_i})"
            )
            output -= (
                nums[top_stack_i]
                * (top_stack_i - left_span_index)
                * (right_span_index - top_stack_i)
            )
            print(f"    Updated output = {output}")
        monotonic_increasing.append(right_span_index)
        print(
            f"  State of the stack after processing index {right_span_index} = {monotonic_increasing}"
        )
    ...
    for right_span_index in range(n + 1):
        ...
        while monotonic_decreasing and (
            ...
            ) if monotonic_decreasing else print(
                f"    Stack is empty so span of {nums[top_stack_i]} starts at -1"
            )
            print(
                f"    output += {nums[top_stack_i]} * ({top_stack_i} - {left_span_index}) * ({right_span_index} - {top_stack_i})"
        ...
        print(
            f"  State of the stack after processing index {right_span_index} = {monotonic_decreasing}"
        )
