...
int main()
{
    ...
    std::vector<VectorDataType> testCases = {
        ...
        {},
        {17},
        {1, std::monostate{}, 12, std::monostate{}, 17, std::monostate{}, 77, 77, 12, std::monostate{}},
    };

    for (auto &nodeData : testCases)
    {
        TreeNode::ptr root = TreeNode::constructBinaryTree(nodeData);
        std::vector<std::vector<TreeNode::datatype>> output = levelOrderTraversal(root);

        std::cout << "The level order traversal output is ";
