...
class Solution(object):
    ...
    def depth_first_search(node: TreeNode, curr_sum: int, target: int) -> bool:
        ...
        """
        if not node:
            print(f"    Empty tree node, returning False\n")
            return False
        if node.left is None and node.right is None:
            comp = curr_sum + node.data == target 
            print(f"    We are at a leaf node, current sum ({curr_sum}) + node data ({node.data}) == target ({target}) is {comp}\n")
            return comp
...
def main() -> int:
    ...
    for node_data, target in test_cases:
        ...
        print(f"There exists {'a' if path_exists else 'no'} path such that sum of all data values equals target = {target}\n\n")
